 ...
 *
 * `apps/web` 側の真にサーバ専用なファイルには `server-only` を入れてビルドで落としているが、
 * `packages/db` は CLI スクリプト・E2E・vitest からも普通の Node として読まれるライブラリなので
...
const SERVER_ONLY_SPECIFIERS = [
  '@skillsheet/db',
  '@skillsheet/db/schema',
  '@skillsheet/db/viewer-rate-limit',
  '@/lib/auth',
  '@/server/session',
