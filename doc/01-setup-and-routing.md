...
パッケージは1つだけで、リポジトリルートがそのまま Next.js のプロジェクトルート。
以前は `apps/web` と `packages/db` の 2 パッケージに分かれていたが、画面が 1 つしか無い
規模で起動位置が 2 か所に散るだけだったので畳んだ（Issue #251）。
...
- mise で Node/pnpm を固定し、リポジトリルート 1 パッケージで完結させる。
