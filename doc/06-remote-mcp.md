...
> **有効化の順序（重要）**: `MCP_ENABLED=true` は **`0006` マイグレーション適用後**に設定すること。
> OAuth プラグインは init 時に `oauth_resource` テーブルへ seed を試みる。テーブル未作成のまま
> 有効化すると seed が例外となり `auth.$context` の初期化自体が失敗し、`/api/mcp` だけでなく
> `/api/auth/*` 全般（ログイン含む）が 500 になる。順序を誤った場合は `MCP_ENABLED` を外すか、
