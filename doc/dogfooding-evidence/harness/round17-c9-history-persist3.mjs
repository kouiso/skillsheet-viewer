...
// 復元は window.confirm で守られている（history-drawer.tsx:44）。
// ハンドラを付けないと Playwright 既定の自動 dismiss で復元が中断される。
const dialogs = [];
p.on('dialog', async (d) => {
  dialogs.push(d.message().replace(/\n+/g, ' / '));
  await d.accept();
});
...
await p.screenshot({ path: `${OUT}/C-9c-before-restore.png` });
...
await p.screenshot({ path: `${OUT}/C-9c-history-drawer.png` });
...
writeFileSync(`${OUT}/C-9-persist3.json`, JSON.stringify(r, null, 2));
