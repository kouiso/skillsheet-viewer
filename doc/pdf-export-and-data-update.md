...
閲覧コード（`VIEWER_CODE`）は例外。本番（Vercel の Production 環境変数）の値は `script/dev-local-stack.sh` の
ローカル既定値と同じ `view123` で、局長の裁定（2026-09-24）により現時点ではパスワード相当として扱わない。
変えるときは Vercel の `VIEWER_CODE` を更新する。
