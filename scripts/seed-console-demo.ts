 ...
 * を ProjectBlockData 形式へマッピングしたもの。本番 Neon（ルート .env.local の DATABASE_URL）に
 * 新規シートとして INSERT する（既存シートは一切変更しない）。確認後は削除する想定。
 *
 * 実行: pnpm exec tsx scripts/seed-console-demo.ts
 */
import { existsSync, readFileSync } from 'node:fs';
import { dirname, resolve } from 'node:path';
import { fileURLToPath } from 'node:url';

import type { BlockInput, CompanyInfo, ProjectItem, ProjectTech } from '../src/db/blocks';

// ルート .env.local から DATABASE_URL / SKILLSHEET_OWNER_ID を読み込む。
...
const { createSheet } = await import('../src/db/skillsheet');
