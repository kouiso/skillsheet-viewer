 ...
 *
 * `src/lib/auth.ts` は `emailAndPassword.disableSignUp: true` のため、
...
// 他の scripts は `.env.local` を読む方式のみだった。この不一致で、
...
function loadBootstrapEnv(): void {
  ...
  const candidates = [resolve(here, '../../../.env'), resolve(here, '../.env.local')];
