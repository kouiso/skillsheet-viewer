...
/** 行全体が `**…**` だけの行。本文中の強調ではなく、直後の段落に付く小見出し。 */
const HEADING_LINE = /^\s*\*\*[^*]+\*\*\s*$/;

/** インライン強調 `**…**` を外す。行まるごとの小見出しは残す。対にならない `**` は残さない。 */
export function unwrapEmphasis(text: string): string {
  return text
    .split('\n')
    .map((line) => {
      // 小見出しの太字まで外すと、直後の本文と同じ見た目になり段落の切れ目が消える。
      // 実データの comment は「**バックエンド**」等を段落の頭に置いて話題を切り替えており、
      // ここを平文にすると複数の話題が地続きの一塊に見える。
      if (HEADING_LINE.test(line)) return line;
