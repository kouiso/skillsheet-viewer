...
export const SortableBlock = ({
  ...
  return (
    <div
      ...
      className="flex flex-col items-start gap-2 rounded-lg border border-border bg-card p-3 shadow-sm sm:flex-row"
    >
      <div className="flex w-full items-center justify-between sm:contents">
        <button
          type="button"
          className="inline-flex h-11 w-11 shrink-0 cursor-grab items-center justify-center rounded text-muted-foreground active:cursor-grabbing sm:order-1"
        ...
        <Button
          ...
          aria-label="ブロックを削除"
          className="mt-0.5 shrink-0 text-muted-foreground hover:text-destructive sm:order-3"
        >
          <Trash2 className="size-4" />
        </Button>
      </div>
      <div className="flex w-full min-w-0 sm:order-2 sm:flex-1">
        {item.type === 'markdown' ? (
          <textarea
            value={item.markdown}
