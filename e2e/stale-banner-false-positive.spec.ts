...
// 元の欠陥（sheet-cache.ts）: unstable_cache の revalidate（60秒）の3倍＝180秒
...
//
// 修正（sheet-cache.ts の withDbHealthCheck）は、fetchedAt の経過を「疑わしいので
...
// 再現できない）ため、`src/server/sheet-cache.test.ts` の withDbHealthCheck 単体テストで
