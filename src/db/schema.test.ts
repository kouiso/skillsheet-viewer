...
describe('文書永続化の型契約', () => {
  it('PG BIGINTをNumberに丸めず受け取り、初期版0を使う', () => {
    ...
    // default は sql`0`（drizzle-kit が 0n を JSON serialize できず db:generate が
    // 落ちるため SQL 式で書く。値としては変わらず初期版 0）。
    expect(skillSheets.revision.default).toBeInstanceOf(SQL);
