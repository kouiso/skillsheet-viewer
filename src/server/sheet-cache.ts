...
function withTimeout<T>(promise: Promise<T>, ms: number): Promise<T> {
  return Promise.race([
    ...
    new Promise<never>((_, reject) => {
      setTimeout(() => reject(new Error(`sheet-cache: live re-check timed out after ${ms}ms`)), ms);
    }),
  ]);
}
...
export async function withDbHealthCheck<C extends { fetchedAt: number }>(
  ...
  try {
    ...
    console.error('sheet-cache: live re-check failed; serving cached (aged) value with stale=true', err);
    return cached;
  }
}
