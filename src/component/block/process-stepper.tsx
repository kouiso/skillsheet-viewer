...
import { PROCESS_LABELS } from '@/db/process';
        ...
        return (
          <div key={label} className="flex min-w-0 flex-col items-center gap-1">
            {!compact && (
              <span
                title={title}
                className={`flex items-center justify-center gap-0.5 break-keep text-center font-mono text-[11px] leading-tight ${isDone ? 'text-accent-text' : 'text-muted-foreground'}`}
              >
