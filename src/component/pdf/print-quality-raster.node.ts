 ...
 *
 * 座標の既定値は print-token.ts の PRINT_SIZE を手で書き写したもの
...
export interface RasterQualityOptions {
  /** ラスタライズの解像度。 */
  dpi: number;
  /** A4 縦の寸法（pt）。print-token.ts の PRINT_SIZE.pageWidth / pageHeight と同じ。 */
  pageWidthPt: number;
  pageHeightPt: number;
  /** ページ内マージン（pt）。print-token.ts の PRINT_SIZE.padTop / padHorizontal と同じ。 */
  marginTopPt: number;
  marginSidePt: number;
  /**
   * 版面（本文が描画されうる領域）の下端。ページ上端からの距離（pt）。
   * print-primitive.tsx の `page` スタイル `paddingBottom: PRINT_SIZE.padBottom + 14` と
   * 同じ実効値（842 − (32 + 14) = 796）。
   */
   ...
   * （print-token.ts、9pt）も覆える値にする。狭いと、カードがたまたま版面の縁で
