...
import { type ProjectBlockData, projectBlockToMarkdown } from '@/db';
...
function registerNodeFonts(): void {
  Font.register({
    ...
    fonts: [
      ...
      // 日本語に true italic は無いため、italic にも同じ字形を割り当てる（本番 font.ts と同じ方針）。
      { src: REGULAR_TTF, fontWeight: 400, fontStyle: 'italic' },
      { src: BOLD_TTF, fontWeight: 700, fontStyle: 'italic' },
    ],
  });
  // 本番の registerPdfFonts()（pdf/font.ts）はブラウザ向け URL でフォント登録するため
...
describe('SkillSheetDocument（実バイト描画）', () => {
  ...
  it(
    ...
    async () => {
      ...
      // script/manual-repro/repro-203-hyphen.tsx で再検証できる）。
...
describe('projectBlockToMarkdown → PDF テキスト層（Issue #242）', () => {
  ...
  // カード自身がページ境界で割れる。block.ts 側で見出し記法を落としているので、
