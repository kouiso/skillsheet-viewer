 ...
 * 案件断片 15）。
 *
 * レガシー markdown 経路（render-node.tsx）は toRenderableText を自前で呼んでおり
 * glyph-verify.node.test.tsx 等で別途守られている。この DB 経路は
 * print-primitive.tsx の PrintText が唯一の共通出口で、そこにサニタイズが無いと
...
describe('DB 印刷経路: 補助面文字（サロゲートペア）', () => {
  ...
  it('氏名・案件名の補助面文字が直後の文字を潰さない（そのままか 〓 に倒れる）', { timeout: 60_000 }, async () => {
    ...
    // 赤くなることを確認済み: print-primitive.tsx の PrintText から
