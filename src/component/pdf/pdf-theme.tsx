 ...
 * ここは値の定義だけを持ち、mdast の走査は render-node.tsx が持つ。
 */
...
import { COLUMN, FONT_SIZE, HEADING_DEPTH, LINE_HEIGHT, PAGE, SPACING } from './layout-metric';

// @react-pdf/textkit の getNodes() は、非空白シラブルの直後が厳密に半角スペース ' ' で
// ない限り hyphenated:true を立て、penalty ノード（既定 hyphenPenalty=600）を追加する。
// splitForHyphenation()（pdf/font.ts）が挟む BREAK_MARKER は ' ' と一致しないため、
...
// 値は design-token.ts を単一の真実として import し、globals.css との乖離を
// design-token.test.ts で機械検証する（PDF は CSS 変数を直接解決できないため）。
export const COLOR = {
...
// ページ幾何・文字サイズ・余白は layout-metric.ts が単一の真実。
export const NUM = {
