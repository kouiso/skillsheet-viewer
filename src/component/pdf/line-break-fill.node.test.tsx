...
describe('和文の段落の改行位置', () => {
  ...
  describe('直しで崩れないこと（いじわるの例）', () => {
    const HAZARDS: { name: string; text: string; widths: number[]; fontSize?: number }[] = [
      ...
      {
        name: '長い英字の後に句点が続き、最後が和文 1〜2 字',
        text: '導入:abcdefghijklmnop。あ。',
        widths: [100, 105, 110, 115, 120, 130],
      },
      // 括弧でくくった 16 字の英字（18 字の切れない塊）が行幅より広い幅（100pt 以下）では、
      // この直しの前から塊の途中で切れて「）」が行頭に来る（main でも同じ）。塊が入る幅だけを測る。
      {
        name: '括弧でくくった長い英字の後が和文 1〜2 字（文の途中）',
        text: '使用したのはライブラリ（abcdefghijklmnop）だ。',
        widths: [105, 110, 115, 120, 130],
      },
      {
        name: '括弧でくくった長い英字の後が和文 1〜2 字（短い文）',
        text: '機能は（abcdefghijklmnop）だ。',
        widths: [105, 110, 115, 120, 130],
      },
      {
        name: '改行しない空白を含む長い英字の連なり',
