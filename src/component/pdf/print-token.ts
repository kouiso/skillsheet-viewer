 ...
 *
 * `src/lib/design-token.ts` は画面の色との一致を `design-token.test.ts` で機械検証している。
 ...
 * スキル一覧ページ（`skill-page.tsx`）に必ず出る。オーナーの「簡易表記はあかん」
...
export const PRINT_YEAR_VISIBLE_CATEGORIES = new Set([
  '言語',
  // 同梱テンプレートから作ったシートはこの分類名で入る。載っていないと年数が
  // 1 つも出ず、画面のスキル表には出ているのに PDF だけ空になる。
  'プログラミング言語',
  'フレームワーク',
  'フレームワーク・ライブラリ',
  'フロントエンド',
