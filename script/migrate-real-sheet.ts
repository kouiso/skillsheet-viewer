 ...
 *
 * 実行:
 *   ドライラン(DB書き込みなし。/tmp/migrated_project_block.json に出力): pnpm exec tsx script/migrate-real-sheet.ts
 *   本番書き込み: pnpm exec tsx script/migrate-real-sheet.ts --write
 */
...
async function main() {
  ...
  if (!process.env.DATABASE_URL) {
    throw new Error('DATABASE_URL が未設定です。.env.local を用意するか、環境変数で渡してください。');
  }

  const { getDb } = await import('../src/db/client');
  const { blocks: blocksTable } = await import('../src/db/schema');
  const { eq, asc } = await import('drizzle-orm');
  const { isStatsBlockData } = await import('../src/db/block');
