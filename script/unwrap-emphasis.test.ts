...
import type { ProjectItem } from '../src/db/block';
    ...
    const blocks: RawDocumentBlock[] = [
      {
        ...
        data: {
          companies: [],
          items: [project('first', '担当は**原文**のまま'), project('second', '変更なし')],
          unknown: '保持',
        },
      },
    ];
    ...
    expect(result.changes).toEqual([
      {
        ...
        projectId: 'first',
        field: 'duties',
        beforeHash: createHash('sha256').update(canonicalJson('担当は**原文**のまま')).digest('hex'),
        afterHash: createHash('sha256').update(canonicalJson('担当は原文のまま')).digest('hex'),
      },
    ]);
